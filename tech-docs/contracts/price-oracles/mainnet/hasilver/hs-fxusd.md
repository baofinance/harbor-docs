# hs fxUSD oracle (haSILVER)

> **Status**: Active  
> **Chain**: Ethereum mainnet (1)  
> **Version**: v4 (leverage)  
> **Address**: `0x70387587a0d3A01a2C75904Dd8CA355a921cd1F3`

## Summary

| Field | Value |
| ----- | ----- |
| **Pair** | hsfxUSD-SILVER/USD |
| **Rate provider** | None — leverage NAV oracle (no collateral rate provider) |
| **Formula / feeds** | Leverage NAV |
| **Type** | leverage |
| **Contract address** | `0x70387587a0d3A01a2C75904Dd8CA355a921cd1F3` |

## Feeds

| Field | Value |
| ----- | ----- |
| **Primary configuration** | Leverage NAV |
| **Rate provider** | None — leverage NAV oracle (no collateral rate provider) |

## Consumers

| Market | Role |
| ------ | ---- |
| — | No Harbor market page yet (inventory / predeploy only) |

## See also

- Chain index: [Mainnet inventory](../../mainnet.md)
- Overview: [Price oracle contracts](../../../price-oracle.md)
